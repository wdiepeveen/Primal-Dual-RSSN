...
function process_RSSN_scaling_S2(λ,scales;postfix="",view_result::Bool=false,path::Union{String,Missing}=missing)
   ...
   if ismissing(path)
      res = load(exp_prefix() *"res-S2" * postfix * ".jld")
   else
      res = load(path *"res-S2" * postfix * ".jld")
      cp(path *"res-S2" * postfix * ".jld",exp_prefix() *"res-S2" * postfix * ".jld")
   end
   ...
   for scale in scales
      if ismissing(path)
         res = load(exp_prefix() *"res-S2-scale-$(scale)" * postfix * ".jld")
      else
         res = load(path *"res-S2-scale-$(scale)" * postfix * ".jld")
         cp(path *"res-S2-scale-$(scale)" * postfix * ".jld",exp_prefix() *"res-S2-scale-$(scale)" * postfix * ".jld")
      end
      data = res["data"]
      ...
      if view_result
         renderAsymptote(exp_prefix()*"original-S2-scale-$(scale).asy", asyExportS2Data; data=data, render=4)
         renderAsymptote(exp_prefix()*"result-lRCPA-S2-scale-$(scale)"*postfix*".asy", asyExportS2Data; data=fR_lrcpa, render=4)
         renderAsymptote(exp_prefix()*"result-RSSN-S2-scale-$(scale)"*postfix*".asy", asyExportS2Data; data=fR_lrssn, render=4)
      else
         asyExportS2Data(exp_prefix()*"original-S2-scale-$(scale).asy";data=data)
         asyExportS2Data(exp_prefix()*"result-lRCPA-S2-scale-$(scale)"*postfix*".asy";data=fR_lrcpa)
         asyExportS2Data(exp_prefix()*"result-RSSN-S2-scale-$(scale)"*postfix*".asy";data=fR_lrssn)
      end
      ...
      # plot!(costfig,[L;lrcpa[:,1]], [TVp;lrcpa[:,2]] .-TVstart, lab="lRCPA scale=$(scale)")
      plot!(costfig,[L;lrssn[:,1]], [TVp;lrssn[:,2]] .-TVstart, lab="PD-RSSN scale=$(scale)")
      plot!(costfig,[0;prior[:,1]], [TVstart;prior[:,2]] .-TVstart,lab="")
      xlims!( (0,L+min(max_xlim,max(length(r_lrssn),round(length(r_lrcpa)/10)))) )
      xlabel!(costfig,"Iterations")
      ylabel!(costfig,"Cost")

      # scatter!(convfig,lrcpa[:,1],lrcpa_cRate, markersize=2, lab="lRCPA scale=$(scale)")
      # scatter!(convfig,lrssn[:,1], lrssn_cRate, markersize=6, lab="PD-RSSN scale=$(scale)")
   ...
   timingfig = plot(scales, timing[2,:,1], lab="PD-RSSN (proposed)")
   # timingfig = plot(scales, timing[1,:,1], lab="lRCPA")
   # plot!(timingfig, scales, timing[2,:,1], lab="PD-RSSN (proposed)")
   ...
   exp_savetable(table, "result-S2"*postfix)
   ...
   exp_savefig(timingfig, "timing-S2"*postfix, true)

   exp_savefig(iterfig, "iter-S2"*postfix, true)
