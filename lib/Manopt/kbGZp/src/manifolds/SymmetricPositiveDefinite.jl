...
is $\frac{n(n+1)}{2}$.
"""
manifoldDimension(M::SymmetricPositiveDefinite) = Int((M.n+1)*M.n/2)
@doc doc"""
    manifoldDimension(x)
...
function parallelTransport(M::SymmetricPositiveDefinite,x::SPDPoint,y::SPDPoint,ξ::SPDTVector)
  ...
  Uf = eig1.vectors
  ν = real(xSqrt*Uf*Sf*transpose(Uf)*(0.5*(tξ+transpose(tξ)))*Uf*Sf*transpose(Uf)*xSqrt)
  return SPDTVector(ν)
end
...
function tangentONB(M::SymmetricPositiveDefinite,x::SPDPoint,ξ::SPDTVector)
    ...
    Ξ = [
        SPDTVector( (i==j ? 1/2 : 1/sqrt(2))*xSqrt*
            ( v[:,i] * transpose(v[:,j])  +  v[:,j] * transpose(v[:,i]) )*xSqrt
        )
